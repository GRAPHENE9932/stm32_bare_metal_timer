.section .text
.global enable_io_porta_clock
.global enable_io_portb_clock
.global set_gpioa_moder
.global set_gpiob_moder
.global write_to_gpioa_odr
.global enable_shift_out_pins
.global shift_out_bits

.equ RCC_BASE, 0x40021000
.equ RCC_AHBENR_OFFSET, 0x00000014
.equ RCC_AHBENR_IOPAEN_BIT, 17
.equ RCC_AHBENR_IOPBEN_BIT, 18
...
.equ GPIOB_MODER_OFFSET, 0x00000000
...
@ Accepts no arguments. Returns nothing.
.type enable_io_portb_clock, %function
enable_io_portb_clock:
    ldr R0, =RCC_BASE + RCC_AHBENR_OFFSET@ R0 stores the word location.
    ldr R1, =1
    lsl R1, R1, #RCC_AHBENR_IOPBEN_BIT  @ R1 stores the bit.
    ldr R2, [R0]                        @ R2 stores the value.
    orr R2, R2, R1
    str R2, [R0]
...
@ 0b11 - Analog mode.
@ Returns nothing.
.type set_gpiob_moder, %function
set_gpiob_moder:
    ldr R2, =GPIOB_BASE + GPIOB_MODER_OFFSET@ R2 stores the word location.
    lsl R0, R0, #1                          @ Multiplying R0 by 2.
    lsl R1, R1, R0                          @ R1 stores the bits to orr with.
    ldr R4, =0xFFFFFFFC
    ldr R3, [R2]
...
@ Accepts no arguments. Returns nothing.
.type enable_shift_out_pins, %function
enable_shift_out_pins:
    push {LR}
